...
protected:
  ...
  double _gset;
public:
  RandomGeneral () {_iset = false; _rmInv = 1.0/double(RAND_MAX);}
 ~RandomGeneral () {};
  double equal () {
  ...
  double gamma (double m, double v) {
    ...
    if      (m <  0.0)       {printf("Random::gamma: m<0");     return 0.0;}
    else if (v <  0.0)       {printf("Random::gamma: v<0");     return 0.0;}
    else if (v == 0.0)       {return m;}
    else if (v <  m*m*1.e-8) {x = m + sqrt(v)*gaussLimit();}
    else {
      alpha = m*m/v;
      beta  = v/m;
      if (alpha < 1.0)       {printf("Random::gamma: alpha<0"); return 0.0;}
      else {
