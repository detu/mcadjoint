...
\pdiff{\Sigma_K^{(i)}}{{S_{\text{w}}}_K^{(i-1)}} &= \begin{split} f^\prime({S_{\text{w}}}_K^{(i-1)}) \cdot \biggr(\indicator(v_\text{tot, x}\lvert_E > 0) v_\text{tot, x}\lvert_E \\ + \indicator(v_\text{tot, x}\lvert_W < 0) v_\text{tot, x}\lvert_W + \indicator(v_\text{tot, y}\lvert_N > 0) v_\text{tot, y}\lvert_N \\+ \indicator(v_\text{tot, y}\lvert_S < 0) v_\text{tot, y}\lvert_S\biggr) \end{split} \\ 
...
\begin{align}
\pdiff{\Sigma_K^{(i)}}{p_K^{(i)}} &= \begin{split}\frac{\Delta t}{h^2} \biggr(T_E^{(i-1)} f(S_\text{w}^{(i-1)})\lvert_E + T_W^{(i-1)} f(S_\text{w}^{(i-1)})\lvert_W \\+ T_N^{(i-1)} f(S_\text{w}^{(i-1)})\lvert_N + T_S^{(i-1)}  f(S_\text{w}^{(i-1)})\lvert_S\biggr) \end{split}\\
